...
<tr>
	<td>组名</td>
	<td colspan='3'><input type='text' name='gname' value='' /></td>
</tr>
<tr>
	<td width='100px'>权限列表</td>
	<td><input type='hidden' name='powerids' id='powerids' value='' />所有用户权限<br/><select id='pids1' multiple style='height:200px;'><?php foreach($data['plist'] as $v){echo "<option value='$v[id]'>$v[title]</option>";}?></select></td>
...
<tr>
	<td>平台列表</td>
	<td width='180px'><input type='hidden' name='areas' id='areas' value='' />所有平台<br/><select id='area1' multiple style='height:200px;'><?php foreach($app->gamelogdb as $k=>$v){echo "<option value='$k'>$k</option>";}?></select></td>
